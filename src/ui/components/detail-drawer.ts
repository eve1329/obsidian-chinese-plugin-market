...
export class PluginDetailDrawer {
	...
	private buildContent() {
		...
		// 复制 README 纯文本（当前显示的译文/原文，去除 Markdown 结构）
		const copyReadmeBtn = readmeHeader.createEl("button", {
			cls: "pt-detail-btn pt-detail-btn--copy-readme",
			attr: { type: "button", "aria-label": this.t("detail.readme.copy") },
		});
		const setCopyReadmeState = (done: boolean) => {
			copyReadmeBtn.empty();
			appendIconText(
				copyReadmeBtn,
				done
					? `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="var(--pt-color-success)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>`
					: `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>`,
				done ? this.t("detail.readme.copy.done") : this.t("detail.readme.copy"),
			);
		};
		setCopyReadmeState(false);
		const copyReadmeEl = copyReadmeBtn as HTMLElement & { _timer?: number };
		copyReadmeBtn.addEventListener("click", () => {
			if (!this.readmeRaw) {
				new Notice(this.t("detail.readme.noContent"));
				return;
			}
			const text = (this.readmeBodyEl?.textContent ?? "")
				.replace(/\r\n/g, "\n")
				.replace(/[ \t]+\n/g, "\n")
				.replace(/\n{3,}/g, "\n\n")
				.trim();
			...
			p.then(
				() => {
					...
					if (copyReadmeEl._timer) window.clearTimeout(copyReadmeEl._timer);
					copyReadmeEl._timer = window.setTimeout(() => {
						setCopyReadmeState(false);
						copyReadmeEl._timer = undefined;
					}, 1200);
				},
				() => new Notice(this.t("card.copy.fail")),
			);
		...
		this._cleanupFns.push(() => {
			if (copyReadmeEl._timer !== undefined) {
				window.clearTimeout(copyReadmeEl._timer);
				copyReadmeEl._timer = undefined;
			}
		});
