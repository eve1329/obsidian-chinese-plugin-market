...
export const STRINGS = {
	...
	// 一键更新（桌面端：下载覆盖 + 重载；移动端不支持，引导回原生设置面板）
	"action.update": { zh: "更新" },
	"action.updateAll": { zh: "全部更新" },
	"action.update.done": { zh: "已更新 {name} 至 v{version}" },
	"action.update.failed": { zh: "更新失败：{msg}" },
	"action.update.noRepo": { zh: "无法获取插件仓库信息，请到设置面板更新" },
	"action.update.mobileBlocked": { zh: "移动端无法自动更新，请到设置面板更新" },
	...
	// 评测台账（阶段5）：独立标签页「我的插件足迹」视图
	"journal.footprint": { zh: "我的插件足迹" },
	"journal.openCmd": { zh: "打开我的插件足迹" },
	"journal.empty": { zh: "还没有评测记录。打开任意插件详情，点头部的「评测」按钮写下你的使用体验，它就会出现在足迹里。" },
	"journal.emptyHintTitle": { zh: "还没有评测记录" },
	"journal.guideInstalled": { zh: "你已安装 / 装过的插件，其安装历史会被自动记录——写完评测后会一起出现在足迹里。" },
	"journal.col.name": { zh: "插件" },
	"journal.col.status": { zh: "状态" },
	"journal.col.rating": { zh: "评分" },
	"journal.col.verdict": { zh: "原因" },
	"journal.col.first": { zh: "首次安装" },
	"journal.col.active": { zh: "最近动态" },
	"journal.col.note": { zh: "备注" },
	"journal.search": { zh: "搜索" },
	"journal.filterAll": { zh: "全部" },
	"journal.filterTried": { zh: "仅装过" },
	"journal.verdictAll": { zh: "全部原因" },
	"journal.copy": { zh: "复制为 Markdown" },
	"journal.copyDone": { zh: "已复制" },
	"journal.copyFail": { zh: "复制失败" },
	"journal.emptyFilter": { zh: "没有匹配的插件" },
	"journal.status.using": { zh: "在用" },
	"journal.status.abandoned": { zh: "弃用" },
	...
	"settings.embedding.sqlite.missing": { zh: "当前安装缺少 SQLite 运行时文件，可直接点击右侧按钮自动补齐。" },
	"settings.embedding.sqlite.repair": { zh: "自动下载并修复" },
	"settings.embedding.sqlite.repairing": { zh: "下载 SQLite 运行时…" },
	"settings.embedding.sqlite.repaired": { zh: "SQLite 运行时已补齐，请重新检测" },
	"settings.embedding.sqlite.repairFailed": { zh: "SQLite 运行时修复失败：{msg}" },
	...
	"settings.prefs.nameDisplay": { zh: "插件标题显示" },
	"settings.prefs.nameDisplay.desc": { zh: "默认显示中文译名还是原标题；无论哪种，点标题都可临时切换另一种语言。" },
	...
	"settings.translateSettings.provider.baidu": { zh: "百度机器翻译（需配置）" },
	"settings.translateSettings.blacklist": { zh: "不翻译的插件 ID" },
	"settings.translateSettings.blacklist.desc": { zh: "逗号或空格分隔的插件 ID，这些插件的设置页保持原文（个别会读取自身 DOM 文案的插件可能需要加入）。" },
	"settings.translateSettings.clearCache": { zh: "清空翻译缓存" },
	"settings.translateSettings.clearCache.desc": { zh: "已翻译过的串会缓存在本地，清空后下次打开重新翻译。" },
	"settings.translateSettings.clearCache.btn": { zh: "清空缓存" },
	"settings.translateSettings.clearCache.tip": { zh: "清除设置页翻译的本地缓存" },
	"settings.translateSettings.cleared": { zh: "已清空设置页翻译缓存" },
	...
	// 鸣谢清单（硬编码常量 src/shared/contributors.ts）
	"settings.thanks": { zh: "鸣谢" },
	"settings.thanks.desc": {
	...
	"settings.profiles.layoutBind": { zh: "绑定布局" },
	"settings.profiles.layoutBind.desc": { zh: "把当前工作区布局（窗口/分屏/停靠）存为该组合的布局快照，应用组合时自动恢复。不绑定则只切插件不切布局。" },
	"settings.profiles.layoutBind.save": { zh: "保存当前布局" },
	"settings.profiles.layoutBind.clear": { zh: "清除布局" },
	"settings.profiles.layoutBound": { zh: "已绑定当前布局到「{name}」" },
	"settings.profiles.layoutCleared": { zh: "已清除「{name}」的布局绑定" },
	"settings.profiles.layoutFail": { zh: "无法读取当前工作区布局" },
	...
	"profiles.current": { zh: "当前生效" },
	"profiles.alreadyActive": { zh: "「{name}」已是当前启用组合" },
	"profiles.saveAs.title": { zh: "另存为启用组合" },
	"profiles.saveAs.desc": { zh: "把当前启用的插件集合存为命名预设，便于一键切换场景。" },
	"profiles.saveAs.menu": { zh: "另存为当前启用集…" },
	"action.cancel": { zh: "取消" },

	// 直链安装（ribbon 菜单「从直链安装插件」）
	"directInstall.title": { zh: "从直链安装插件" },
	"directInstall.desc": {
	...
	"directInstall.needHttps": { zh: "插件代码必须走 https 下载" },
	"directInstall.fetchFail": { zh: "取不到：HTTP" },
	"directInstall.ghNotFound": { zh: "GitHub 上找不到 {repo}（仓库不存在、已改名或为私有，请检查用户名/仓库名拼写）" },
	"directInstall.ghSuggest": { zh: "GitHub 上找不到 {repo}；你是想安装 {sug} 吗？请检查拼写后重试" },
	"directInstall.ghNoManifest": { zh: "找到仓库 {repo}，但它根目录没有 manifest.json——它可能不是可安装的插件仓库" },
	"directInstall.ghNoMain": { zh: "仓库 {repo} 的源码树和 Release 里都没有 main.js（作者未发布构建产物，无法安装）" },
	"directInstall.badManifest": { zh: "manifest.json 不合法（id 只能含字母数字 _ . - 且不能以 . 开头）" },
	"directInstall.minApp": { zh: "它需要 Obsidian {v} 或更高版本" },
	"directInstall.emptyMain": { zh: "main.js 是空的" },
	"directInstall.noPluginsApi": { zh: "无法访问插件管理器（app.plugins 不可用）" },
	"directInstall.enableFailed": { zh: "已下载，但 Obsidian 没能启用它，请在第三方插件列表里查看" },
	"directInstall.done": { zh: "已安装 {name} v{version}" },
	...
	"settings.tm.openFolder.tip": { zh: "用系统文件管理器打开当前记忆库所在位置（桌面端）" },
	"settings.tm.openFolder.failed": { zh: "无法打开文件夹：{path}（移动端可能不支持）" },
	...
	"notice.install.disabled": { zh: "「{name}」已禁用（插件文件已保留，重启后仍保持禁用）" },
